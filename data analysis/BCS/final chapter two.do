...
gen sexbirth=.
replace sexbirth = 1 if (a0255==1)
replace sexbirth = 0 if (a0255==2)

label define sex_lbl 1"Male" 0"Female"
label values sexbirth sex_lbl

tab sexbirth

keep bcsid sexbirth

save newbcs_test, replace
...
gen econact=.
replace econact=1 if(va86sep==5)
replace econact=1 if(va86sep==6)
replace econact=1 if(va86sep==7)

replace econact=2 if(va86sep==4)

replace econact=3 if(va86sep==3)

replace econact=4 if(va86sep==1)
replace econact=4 if(va86sep==2)

label define econact_lbl 1"Employment" 2"Education" 3"Training & Apprenticeships" 4"Unemployment & OLF"
label values econact econact_lbl
...
keep bcsid female econact

merge m:1 bcsid using newbcs_test
drop _merge

gen sex = sexbirth 
replace sex = female if missing(sexbirth)
label values sex sex_lbl 

tab sex

merge m:m bcsid using newbcs_test
drop _merge 

keep bcsid econact sex  
...
merge 1:m bcsid using newbcs_test
drop _merge 

keep bcsid sex econact nssecdom2000 nssecdom90 rgscdom2000 rgscdom90 camsisdom2000 camsisdom90
...
list bin26 obinary in 1/100


gen obin= bin26 
replace obin = obinary if missing(bin26)
...
use"G:\Stata data and do\BCS\BCS Economic Activity Dataset\UKDA-6943-stata\stata\stata13\bcs70_activity_histories_eul"

egen year_month = group(JSTYR JSTMTH), label

sort BCSID year_month
by BCSID: gen seq = _n

gen unique_id = BCSID + "_" + string(year_month) + "_" + string(seq)

drop JSTYR JSTMTH seq

reshape wide JACTIV, i(unique_id) j(year_month)
...
gen finalecon = econact
replace finalecon = econ201 if missing(econact)

tab finalecon 

label variable finalecon "Economic Activity of Respondent"
...
keep bcsid finalecon obin sex htenure nssecdom2000 nssecdom90 rgscdom2000 rgscdom90 camsisdom2000 camsisdom90 crecords

count


misstable summarize finalecon obin htenure sex nssecdom2000 nssecdom90 rgscdom2000 rgscdom90 camsisdom2000 camsisdom90 

misstable patterns finalecon obin htenure sex nssecdom2000 nssecdom90 rgscdom2000 rgscdom90 camsisdom2000 camsisdom90 

misstable patterns finalecon obin htenure sex nssecdom2000 nssecdom90 rgscdom2000 rgscdom90 camsisdom2000 camsisdom90 , frequency

save bcs_final_raw, replace


keep if !missing(finalecon, obin, htenure, sex, nssecdom2000, nssecdom90, rgscdom2000, rgscdom90, camsisdom2000, camsisdom90 )
...
label define econbin1_lbl 1"Continue Schooling" 0"Don't Continue Schooling"
label values econbin econbin1_lbl

tab econbin

logit econbin obin sex htenure ib(3).nssecdom2000

mlogit finalecon obin sex htenure ib(3).nssecdom2000
