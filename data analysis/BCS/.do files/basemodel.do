...
graph export "bcsnssecmargins.png", width(6000) replace


margins obin, atmeans saving(file6, replace)
marginsplot, recast(scatter) ciopt(color(black)) recastci(rspike) title("Educational Attainment, Predictive Margins", size(small)) xtitle("Educational Attainment", size(small)) ytitle("Continuing Schooling", size(small)) plotopts(msymbol(Oh) mcolor(black) lcolor(black) lpattern("l")) ylabel(, labsize(small)) xlabel(0 "<5" 1 "≥5") note("Data Source: BCS, N=1,574", size(vsmall)) caption("NS-SEC, Sex, and Housing Tenure also included in Model", size(vsmall)) name(main2, replace) 

graph export "bcsobinmargins.png", width(6000) replace

margins sex, atmeans saving(file11, replace)
marginsplot, recast(scatter) ciopt(color(black)) recastci(rspike) title("Sex, Predictive Margins", size(small)) xtitle("Sex", size(small)) ytitle("Continuing Schooling", size(small)) plotopts(msymbol(Oh) mcolor(black) lcolor(black) lpattern("l")) ylabel(, labsize(small)) xlabel(0 "Female" 1 "Male") note("Data Source: BCS, N=1,574", size(vsmall)) caption("Educational Attainment, NS-SEC, and Housing Tenure also included in Model", size(vsmall)) name(main2, replace) 

graph export "bcssexmargins.png", width(6000) replace

margins tenure, atmeans saving(file16, replace)
marginsplot, recast(scatter) ciopt(color(black)) recastci(rspike) title("Housing Tenure, Predictive Margins", size(small)) xtitle("Housing Tenure", size(small)) ytitle("Continuing Schooling", size(small)) plotopts(msymbol(Oh) mcolor(black) lcolor(black) lpattern("l")) ylabel(, labsize(small)) xlabel(0 "Own" 1 "Don't") note("Data Source: BCS, N=1,574", size(vsmall)) caption("Educational Attainment, Sex, and NS-SEC also included in Model", size(vsmall)) name(main2, replace) 

graph export "bcstenuremargins.png", width(6000) replace
