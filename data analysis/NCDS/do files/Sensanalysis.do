...
coefplot (logitnsseca, ciopt(color(red%100)) msymbol(oh) mcolor(red%100) lcolor(red) drop(_cons) label("NS-SEC model")) ///
         (logitcamsiscca, ciopt(color(blue%100)) msymbol(dh) mcolor(blue%100) lcolor(blue) drop(_cons) offset(0.1) label("CAMSIS model")) ///
         (logitrgsccca, ciopt(color(gold%100)) msymbol(th) mcolor(gold%100) lcolor(gold) drop(_cons) offset(-0.1) label("RGSC model")), ///
         xline(0) ///
         title("Coefficient Plots of Logistic Regression Results", size(small) color(black)) ///
         subtitle("Betas and CIs of Logit model analysing structural impacts on continuing schooling", size(small) color(black)) ///
         note("Data Source: NCDS, N=8,411", size(vsmall) color(black)) ///
